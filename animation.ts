...
function buildIdle(boneNames: string[]): RiveAnimationDef {
  ...
  return { name: 'Idle', fps, durationFrames: totalFrames, loopType: 1, tracks };
}
...
function buildWalk(boneNames: string[]): RiveAnimationDef {
  ...
  return { name: 'Walk', fps, durationFrames: totalFrames, loopType: 1, tracks };
}
...
function buildWave(boneNames: string[]): RiveAnimationDef {
  ...
  return { name: 'Wave', fps, durationFrames: totalFrames, loopType: 0, tracks };
}
...
function buildJump(boneNames: string[]): RiveAnimationDef {
  ...
  return { name: 'Jump', fps, durationFrames: totalFrames, loopType: 0, tracks };
}
...
function buildDeath(boneNames: string[]): RiveAnimationDef {
  ...
  // Limb collapse
  for (const limb of ['l_upper_arm', 'r_upper_arm', 'l_upper_leg', 'r_upper_leg']) {
    if (hasBone(limb)) {
      tracks.push({
        boneName: limb,
        property: 'rotation',
  ...
  return { name: 'Death', fps, durationFrames: totalFrames, loopType: 0, tracks };
}

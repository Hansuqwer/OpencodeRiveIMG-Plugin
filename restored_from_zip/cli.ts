...
function findNearestExistingAncestor(targetPath: string): string {
  ...
  while (!existsSync(current)) {
    ...
    if (parent === current) {
      throw new Error(
        `Unable to resolve existing parent directory for path: ${targetPath}`,
      );
    }
    current = parent;
  }
...
export function resolvePathWithinBase(
  ...
  if (options.mustExist) {
    ...
    if (!isWithinBase(baseRealPath, candidateRealPath)) {
      throw new Error(
        `Path traversal denied: "${userPath}" escapes base directory`,
      );
    }
    return resolved;
  }
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    switch (arg) {
      ...
      case '--output-bundle':
        ...
        break;
      case '--mesh-density':
        result.meshDensity = parseNumber(
          expectValue(args, index, '--mesh-density'),
          '--mesh-density',
        );
        index += 1;
        break;
      case '--animations':
        result.animations = parseAnimationList(
          expectValue(args, index, '--animations'),
        );
        index += 1;
        break;
      case '--width':
        result.width = parseNumber(
          expectValue(args, index, '--width'),
          '--width',
        );
        index += 1;
        break;
      case '--height':
        result.height = parseNumber(
          expectValue(args, index, '--height'),
          '--height',
        );
        index += 1;
        break;
...
async function runConvertCli(args: string[]): Promise<void> {
  ...
  const defaultOutputPath = path.join(
    path.dirname(input),
    `${path.parse(input).name}.rivebundle`,
  );
  const output =
    parsed.output !== undefined
      ? resolvePathWithinBase(baseDir, parsed.output, {
          mustExist: false,
          purpose: 'output',
        })
      : resolvePathWithinBase(baseDir, defaultOutputPath, {
          mustExist: false,
          purpose: 'output',
        });
...
function printResult(result: PipelineResult): void {
  ...
  console.log(
    `Primary rig   : ${result.primaryArtboardId} (${result.skeletonType})`,
  );
  console.log(`Artboards     : ${result.artboardCount}`);
  console.log(
    `Primary mesh  : ${result.vertexCount} verts / ${result.triangleCount} tris`,
  );
...
export const imageToRiveTool: OpenCodeTool = {
  name: 'image_to_rive',
  description:
  ...
  async execute(params, ctx) {
    ...
    const options: PipelineOptions = {
      ...
      outputBundle,
      meshDensity:
        typeof params.mesh_density === 'number'
          ? params.mesh_density
          : undefined,
      animations: Array.isArray(params.animations)
        ? params.animations.filter((item): item is AnimationPreset =>
            ANIMATION_PRESETS.includes(item as AnimationPreset),
          )
        : undefined,
      artboardWidth:
        typeof params.artboard_width === 'number'
          ? params.artboard_width
          : undefined,
      artboardHeight:
        typeof params.artboard_height === 'number'
          ? params.artboard_height
          : undefined,
      sheetMode:
        typeof params.sheet_mode === 'string' &&
        SHEET_MODES.has(params.sheet_mode)
...
export const initTool: OpenCodeTool = {
  name: 'rive_init',
  description:
  ...
  parameters: {
    ...
    properties: {
      global: {
        type: 'string',
        description:
          'Set to "true" to install globally in ~/.config/opencode/ instead of the current project.',
        enum: ['true', 'false'],
        default: 'false',
      },
      force: {
        type: 'string',
        description:
          'Set to "true" to overwrite existing config and AGENTS.md section.',
        enum: ['true', 'false'],
        default: 'false',
      },
    },
  ...
  async execute(params, ctx) {
    ...
    try {
      ...
      return {
        success: true,
        message: 'Rive plugin initialized. See AGENTS.md for capabilities.',
      };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
...
async function runInit(args: string[]): Promise<void> {
  ...
  if (existsSync(agentsPath)) {
    ...
    if (existing.includes(AGENTS_MD_MARKER_START)) {
      if (isForce) {
        ...
        console.log(
          `⊘ Rive section already exists in ${agentsPath} (use --force to replace)`,
        );
      }
    } else {
      // Append new section
      ...
      writeFileSync(
        agentsPath,
        `${existing + separator + markedSection}\n`,
        'utf8',
      );
  ...
  console.log(
    `  Python deps  : ${depsOk ? 'OK' : 'MISSING — run: pip install -r requirements.txt'}`,
  );
  console.log(
    `  Pipeline     : ${scriptsFound ? 'ready' : 'scripts not found at expected path'}`,
  );
  console.log('═══════════════════════════════════════════════════');
  ...
  if (!scriptsFound) {
    console.log('');
    console.log(
      'Python scripts (segment.py, pose_estimate.py) were not found next to the CLI.',
    );
    console.log(
      'If installed globally via npm, ensure the package includes these files.',
    );
  }

  console.log('');
  console.log(
    'Done. Your AI model can now see the Rive plugin capabilities in AGENTS.md.',
  );
}
